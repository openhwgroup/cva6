...
def tandem_postprocess(tandem_report, target, isa, test_name, log, testlist, iss,
  ...
  process_verilator_sim_log(log, log + ".csv", exit_on_ecall=exit_on_ecall)
...
def load_config(args, cwd):
  ...
  if not args.custom_target:
    ...
    if base in ("cv64a6_imafdch_sv39", "cv64a6_imafdch_sv39_wb"):
      args.mabi = "lp64d"
      args.isa  = "rv64gch_zba_zbb_zbs_zbc_zicbom"
    elif base in ("cv64a6_imafdc_sv39_wb",):
      args.mabi = "lp64d"
      args.isa  = "rv64gc_zba_zbb_zbs_zbc_zicbom"
    elif base in ("cv64a6_imafdc_sv39", "cv64a6_imafdc_sv39_hpdcache", "cv64a6_imafdc_sv39_hpdcache_wb"):
      args.mabi = "lp64d"
      args.isa  = "rv64gc_zba_zbb_zbs_zbc_zicbom_zbkb_zbkx_zkne_zknd_zknh"
    elif base == "cv32a60x":
      args.mabi = "ilp32"
      args.isa  = "rv32imc_zba_zbb_zbs_zbc_zicbom"
      args.priv  = "m"
    elif base == "cv32a65x":
      args.mabi = "ilp32"
      args.isa  = "rv32imc_zba_zbb_zbs_zbc_zicbom"
