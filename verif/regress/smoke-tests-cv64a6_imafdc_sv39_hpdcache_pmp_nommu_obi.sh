...
for test in "${SV39_PRIV_TESTS[@]}"; do
  python3 cva6.py --iss_timeout=30000 \
    ...
    --target cv64a6_imafdc_sv39_hpdcache_pmp_nommu_obi \
    --iss=$DV_SIMULATORS $DV_OPTS \
    --linker=../tests/custom/privtests/priv/config/link.ld
done
python3 cva6.py --iss_timeout=30000 --testlist=../tests/testlist_riscv-compliance-cv64a6_imafdc_sv39.yaml --test rv32i-I-ADD-01 --iss_yaml cva6.yaml --target cv64a6_imafdc_sv39_hpdcache_pmp_nommu_obi --iss=$DV_SIMULATORS $DV_OPTS
python3 cva6.py --iss_timeout=30000 --testlist=../tests/testlist_riscv-tests-cv64a6_imafdc_sv39-p.yaml --test rv64ui-p-add --iss_yaml cva6.yaml --target cv64a6_imafdc_sv39_hpdcache_pmp_nommu_obi --iss=$DV_SIMULATORS $DV_OPTS
python3 cva6.py --iss_timeout=30000 --testlist=../tests/testlist_riscv-arch-test-cv64a6_imafdc_sv39.yaml --test rv64i_m-add-01 --iss_yaml cva6.yaml --target cv64a6_imafdc_sv39_hpdcache_pmp_nommu_obi --iss=$DV_SIMULATORS $DV_OPTS  --linker=../../config/gen_from_riscv_config/linker/link.ld
python3 cva6.py --iss_timeout=30000 --testlist=../tests/testlist_custom.yaml --test custom_test_template --iss_yaml cva6.yaml --target cv64a6_imafdc_sv39_hpdcache_pmp_nommu_obi --iss=$DV_SIMULATORS $DV_OPTS
python3 cva6.py --iss_timeout=30000 --c_tests ../tests/custom/hello_world/hello_world.c --iss_yaml cva6.yaml --target cv64a6_imafdc_sv39_hpdcache_pmp_nommu_obi --iss=$DV_SIMULATORS --gcc_opts="$CC_OPTS -nostdlib -lgcc" $DV_OPTS --linker=../../config/gen_from_riscv_config/linker/link.ld
make -C ../.. clean
make clean_all
