...
Interrupt and debug events detected by the RTL model are injected into the reference model at the beginning of each step. The values of microarchitecture-dependent CSRs such as clock cycle counters (`cycle`/`cycleh`, `mcycle`/`mcycleh`) are injected at the end of the reference model step since the RTL values of these registers correspond to RTL state *after* the corresponding RTL event.
...
Control of single/tandem operation mode is achieved using the SystemVerilog macro `SPIKE_TANDEM`.  If defined to a non-zero ("true") value, it enables the tandem configuration in the RTL simulation by instantiating and connecting the Spike wrapper, and by activating the step-and-compare functions.
...
Option `+config_file` should only be used when an appropriate configuration file exist, and should be omitted otherwise.
...
To limit the impact on the original code base, Spike modifications rely on "shim" (adapter) classes that add extra functionality in two ways:
...
Spike execution model was modified to run a single-instruction step function instead of the performance-optimized "instruction batch" mode that executes hundreds to thousands of instructions without yielding the control to the environment of the simulated design.
...
A machine-processable Yaml report file summarizing the discrepancies detected during the simulation is stored in a separate report file, by default `verif/sim/out_<date>/<sim-config>_sim/<test-name>.<target>.log.yaml`.  It contains the configuration of the simulated design, the number of mismatches that were found (if any) and a list of entries describing the successive mismatches.
...
This following is the corresponding fragment from the Yaml report, including the configuration and status summary of the simulation run:
...
Given the interests expressed in the CVA6 community, one of posssible directions of evolution for the OpenHW Group tandem verification platform is to support single-clock multi-core platforms. The Spike platform abstraction supports already multiple heterogeneous cores and multiple memory regions, meaning that a reference model is already available provided the targeted design does not involve complex memory access structures (multilevel caches, memory access networks, etc.)  Also, iIndividual core configurations can already be expressed in the current parameterization scheme, but addtional work will be needed to handle more complex memory maps.  If non-trivial memory architectures need to be simulated, the bulk of the effort will be devoted to the modeling of such structures which is beyond the scope of the project.

Another direction for future work is to substitute Spike with another reference model.  While `Sail` (https://github.com/riscv/sail-riscv) appears as a potential candidate, it is currently unclear how much effort will be required in order to achieve a level of functionality sufficiently rich to run complete programs and expose an RVFI-type interface for tandem verification.
