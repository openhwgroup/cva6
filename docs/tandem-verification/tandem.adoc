...
Tandem-based functional verification of a RTL design consists in a lockstep comparison of the behavior of an RTL implementation against the behavior of a reference model (usually an Instruction Set Simulator, or ISS), both executing the same programs from the same initial state.  In opposition to post-execution trace comparison, the tandem approach enables immediate detection of differences in observable behavior of the RTL implementation and the reference model, thus greatly simplifying the investigation of the root causes of the discrepancies.
...
RVFI events are collected from both RTL and reference model side, and a scoreboard is used to compare the states of both simulations.
...
In order to ease the integration with RTL simulators, the tandem infrastructure uses a limited subset of UVM to provide uniform SystemVerilog interface across all RTL simulators, whether commercial or open-source.
...
Spike was modified to run a single-instruction step function instead of the performance-optimized "instruction batch" mode that executes hundreds to thousands of instructions without yielding the control to the environment of the simulated design.
...
* three ways of controlling Spike simulator behavior:
...
* ability to inject CSR values for architecture-dependent CSRs (`cycles`/`cyclesh`, `mcycles`/`mcyclesh`, `mip` etc.)
