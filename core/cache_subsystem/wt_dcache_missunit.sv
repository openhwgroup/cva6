...
#(
  ...
  if (CVA6Cfg.RVA) begin
    ...
    /*
    logic endian_data[63:0];
    assign endian_data = (mbe_i)? {<<8{amo_rtn_mux}} : amo_rtn_mux;
    always_comb begin 
      if (~mbe_i) begin
        endian_data = amo_rtn_mux;
      end else begin
        if (amo_req_i.size==2'b10) begin
          endian_data = (amo_req_i.operand_a[2])? {{<<8{amo_rtrn_mux[63:32]}}, {amo}}: ;
        end else begin
          endian_data = {<<8{amo_rtrn_mux}};
        end
    end
*/
    ...
    };  // +7 to the base address to find the sign bit, as in BE the Most Significant Byte is always at the lowest address.
    ...
    assign amo_resp_o.result = (mbe_i == 1) ? amoResult_BE : amoResult_LE;
