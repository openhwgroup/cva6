...
def factorizer(yaml_data):
                ...
                if match :
                    ...
                    if privname and match.group(1) == privname.group(1) :
                        ...
                        if regname:
                            ...
                            modified_data['address']=f"{str(start_address)}-{str(regadress[-1])}"
                        ...
                        if suffix_name:
                            suffix_name = sorted(suffix_name , key = lambda x:int(x.lstrip
                            (field_suffix[0]).rstrip('h')))
                            ...
                            modified_data['address'] = f"{str(suffix_address[0])}-{str(suffix_address[-1])}"
                            new_regname.append(f"{field_suffix[0]}[{suffix_number[0]}-{suffix_number[-1]}]h")
                            data.append(modified_data)
                        suffix_name = []
                        field_suffix =[]
                        regdescr= []
    ...
    if suffix_name:
        ...
        modified_data =yaml_data['hart0'][suffix_name[0]].copy()
        modified_data['description'] = desc
        modified_data['address'] = f"{str(hex(int(suffix_address[0],16)))}-{str(suffix_address[-1])}"
        new_regname.append(f"{field_suffix[0]}[{suffix_number[0]}-{suffix_number[-1]}]h")
        data.append(modified_data)
        suffix_name = []
        field_suffix =[]
