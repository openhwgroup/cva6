...
def isa_recursive_update(original_dict, isa_dict):
    """
    Gets the data of the isa template and update the value of sub key in isa_template(ex: format , legal_values)
...
def isa_filter(parsed_data , updater, template):
    """
    create dict of filtred data from isa_template  based on supported extensions read from ISA String from RISC-V 	Yaml file 
    ...
    """
    hart0_data = parsed_data['hart0']
 
# Define the regular expression pattern
    isa_regex = re.compile(r"""
...
# Use search to find the pattern in the string 
    match = isa_regex.match(hart0_data["ISA"])
    extensions =[]
    
# If a match is found, extract the extensions
    if match:
    
      characters = [char for char in match.group(2)]
...
# Printing subset names
    ...
    for ext in missing_extensions:
        subset_data[ext] = {'Description': f"No info found yet for extension {ext}",'Subset_Name' : ext}
    return subset_data
