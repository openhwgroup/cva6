...
def find_pr(branch, prs):
    match = re.search(
        r"(.*)_PR_([a-zA-Z0-9](?:[a-zA-Z0-9]|[-_](?=[a-zA-Z0-9])){0,38})", branch
    )
    if match:
        label = f"{match.group(2)}:{match.group(1)}"
        for pull_req in prs:
            if label == pull_req["head"]["label"]:
                return pull_req
    return None
...
pipeline_creation_timestamp = int(
    datetime.datetime.strptime(
        os.environ["CI_PIPELINE_CREATED_AT"], "%Y-%m-%dT%H:%M:%S%z"
    ).timestamp()
)
...
if workflow_type == "github":  # (from wrapper)
    ...
    workflow_repo = (
        ...
        else os.environ["CI_COMMIT_REF_NAME"]
    )  # cvv or cva6
...
pipeline = {
    ...
    "env": {
        "cva6": {"sha": cva6_sha, "branch": cva6_branch},
        "core-v-verif": {"sha": cvv_sha, "branch": cvv_branch},
    },
...
search_pattern = "artifacts/reports/*.yml"
file_list = glob.glob(search_pattern)


for filepath in file_list:
    with open(filepath, "r", encoding="utf-8") as job_report:
...
filename = f"pipeline_report_{workflow_uid}.yml"

pipeline_report_dir = "cva6" if source_branch == "master" else source_branch

with open(f"artifacts/reports/{filename}", "w+", encoding="utf-8") as f:
...
cp artifacts/reports/{filename} .gitlab-ci/dashboard_tmp/pipelines_{pipeline_report_dir}/
